        ...
        <div className=" shadow-md shadow-[#6be4b9] border border-[#6be4b9] w-9/12 h-full flex flex-col rounded-lg mt-5 ml-2">
          ...
          <div className="m-3 p-2 w-auto rounded-lg mb-4 flex items-center bg-[#6be4b9] space-x-4">
            <img
              ...
              className="w-10 h-10 rounded-full border border-purple-500 shadow-md"
            />
            <h1 className="font-bold text-gray-800 text-xl ">{currentUser.name}</h1>
          </div>

          {/* Scrollable posts */}
          <div className="flex-1 items-center ml-56 overflow-y-auto pr-2">
            {posts.length === 0 ? (
              <p className="text-gray-400 pt-10 text-center">Одоогоор пост алга байна.</p>
            ) : (
              posts.map((post, index) => (
                <div key={index} className="mb-4 w-96 p-4 bg-[#313f40] rounded-md shadow-md shadow-[#6be4b9]">
                  <h3 className="text-lg font-semibold text-white">{post.title}</h3>
                  <p className="text-gray-300">{post.body}</p>
                  <p className="text-sm text-gray-500">Багшийн ID: {post.teacher_id}</p>
                </div>
              ))
            )}
          </div>
